...
public class RainBenchmark
{
    ...
    public IEnumerator Measure()
    {
        ...
        else if (variant == "Compare040")
        {
            yield return MeasureVariant("Previous030");
            yield return MeasureVariant("Current040");
        }
    ...
    private IEnumerator MeasureVariant(string variant)
    {
        ...
        if (variant == "Reference" || variant == "Previous030")
        {
            var shader = Shader.Find(variant == "Previous030" ? "Hidden/jlinkdev/Volumetric Fog and Rain Previous030" : "Hidden/jlinkdev/Volumetric Fog and Rain Reference");
        ...
        var sun = new GameObject("Benchmark directional light").AddComponent<Light>();
        sun.type = LightType.Directional; sun.intensity = 1;
        sun.transform.rotation = Quaternion.Euler(45,-30,0); objects.Add(sun.gameObject);
        ...
        try
        {
            ...
            if (variant != "Reference") scenarios.AddRange(new[] { "bounded", "sheltered", "fully_dry", "fog_global", "fog_bounded", "rain_feather", "fog_feather", "fog_features" });
            if (variant == "Previous030" || variant == "Current040") scenarios = new List<string> { "default", "no_noise", "fog_global", "fog_bounded" };
            foreach (string scenario in scenarios)
            {
                profile.renderMode = scenario.StartsWith("fog_") ? RainRenderMode.FogOnly : RainRenderMode.RainAndFog;
                wetBox.enabled = scenario == "bounded" || scenario == "fog_bounded" || scenario.EndsWith("feather") || scenario == "fog_features";
                wetBox.featherDistance = scenario.EndsWith("feather") || scenario == "fog_features" ? 2 : 0;
                dryBox.featherDistance = scenario == "fog_features" ? .75f : 0;
                profile.fogHeightFalloff = scenario == "fog_features" ? .15f : 0;
                profile.directionalScattering = scenario == "fog_features" ? .15f : 0;
                profile.fogNoiseVelocity = scenario == "fog_features" ? new Vector3(.2f,0,.1f) : Vector3.zero;
                profile.extent = wetBox.enabled ? RainExtent.VolumesOnly : RainExtent.Unbounded;
                dryBox.enabled = scenario == "sheltered" || scenario == "fully_dry" || scenario == "fog_features";
                ...
                for (int i=0;i<420;i++)
                {
                    ...
                    sw.Stop();
                    if (recorder==null) { recorder=Recorder.Get("Volumetric Fog and Rain"); recorder.enabled=true; }
                    yield return null;
                    if (i>=120)
                    {
