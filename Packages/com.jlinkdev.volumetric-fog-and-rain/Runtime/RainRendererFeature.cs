...
{
    public enum RainInjectionPoint { BeforePostProcessing, BeforeTransparentCapture, BeforeTransparents, AfterPostProcessing }
    public enum FogOutputMode { PreserveSceneAlpha, Premultiplied }

    [DisallowMultipleRendererFeature("Volumetric Fog and Rain")]
    ...
    public sealed class RainRendererFeature : ScriptableRendererFeature
    {
        ...
        [Tooltip("Default is after transparents and before post-processing. Capture stage is after skybox, before URP opaque-color copy.")]
        public RainInjectionPoint injectionPoint;
        [Tooltip("Premultiplied expects premultiplied source RGB and updates opacity for compositing onto another background. Default preserves scene alpha.")]
        public FogOutputMode outputMode;
        public static RenderPassEvent InjectionEvent(RainInjectionPoint point)
        {
            switch (point)
            {
                case RainInjectionPoint.BeforeTransparentCapture: return RenderPassEvent.AfterRenderingSkybox;
                case RainInjectionPoint.BeforeTransparents: return RenderPassEvent.BeforeRenderingTransparents;
                case RainInjectionPoint.AfterPostProcessing: return RenderPassEvent.AfterRenderingPostProcessing;
                default: return RenderPassEvent.BeforeRenderingPostProcessing;
            }
        }
        ...
        public override void Create()
        {
            ...
            if (shader == null) shader = Shader.Find("Hidden/jlinkdev/Volumetric Fog and Rain");
        ...
        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            ...
            var settings = cameraData.isSceneViewCamera ? pass.SceneSettings(sceneViewProfile) :
                (rain != null && rain.isActiveAndEnabled ? rain.ResolveSettings() : null);
            if (settings == null || (!settings.HasRain && !settings.HasFog) || (!cameraData.isSceneViewCamera && rain.intensity <= 0)) return;
            pass.renderPassEvent = InjectionEvent(injectionPoint);
        ...
        private bool CollectVolumes(Camera camera, bool bounded, PassVolumeData data)
        {
            ...
            foreach (var box in RainBoxVolume.Active)
            {
                ...
                data.feather |= box.featherDistance > 0;
                ...
                if (box.ExcludesRain)
                {
                    if (data.dryCount < 8)
                    {
                        data.dryBoxes[data.dryCount] = PadExclusion(matrix.inverse);
                        data.dryFeathers[data.dryCount] = FeatherData(data.dryBoxes[data.dryCount], box.featherDistance);
                    }
                    data.dryCount++;
                }
                else
                {
                    if (data.rainCount < 4)
                    {
                        data.rainBoxes[data.rainCount] = matrix.inverse;
                        data.rainFeathers[data.rainCount] = FeatherData(matrix.inverse, box.featherDistance);
                    }
                    data.rainCount++;
                }
            }
            ...
            if (overflow && !warnedVolumeLimit)
                Debug.LogWarning("Volumetric Fog and Rain supports at most 4 visible rain boxes and 8 visible exclusion boxes per camera. Rain is skipped for this camera until the limit is respected, so dry regions are never silently ignored.", this);
            warnedVolumeLimit |= overflow;
            return !overflow && (!bounded || data.rainCount > 0);
        }

        private static Vector4 FeatherData(Matrix4x4 inverse, float feather)
        {
            return new Vector4(1f / ((Vector3)inverse.GetRow(0)).magnitude,
                1f / ((Vector3)inverse.GetRow(1)).magnitude,
                1f / ((Vector3)inverse.GetRow(2)).magnitude, Mathf.Max(0, feather));
        }
        ...
        private sealed class PassVolumeData
        {
            ...
            public readonly Vector4[] rainFeathers = new Vector4[4];
            public readonly Vector4[] dryFeathers = new Vector4[8];
            public bool feather;
        ...
        private sealed class RainPass : ScriptableRenderPass
        {
            ...
            private readonly RainSettings sceneSnapshot = new RainSettings();
            private double sceneLastTime, sceneFogX, sceneFogY, sceneFogZ;
            private bool sceneHasTime;
            public RainSettings SceneSettings(RainProfile profile)
            {
                if (profile == null) return null;
                sceneSnapshot.CopyFrom(profile);
                return sceneSnapshot;
            }
            private Vector3 SceneFogOffset(Vector3 velocity, float scale, double time)
            {
                double dt = sceneHasTime ? time - sceneLastTime : time;
                sceneLastTime = time; sceneHasTime = true;
                sceneFogX += velocity.x * dt; sceneFogY += velocity.y * dt; sceneFogZ += velocity.z * dt;
                return new Vector3(WrapNoise(sceneFogX * scale), WrapNoise(sceneFogY * scale), WrapNoise(sceneFogZ * scale));
            }
            private static float WrapNoise(double x) => (float)(x - System.Math.Floor((x + 32768.0) / 65536.0) * 65536.0);
            ...
            private sealed class PassData
            {
                ...
                public Vector4 fogColor, fogAppearance, fogDistance, fogHeight, fogOffset, lightColor, lightDirection;
                public bool premultiplied, advancedFog;
            }
            ...
            public override void RecordRenderGraph(RenderGraph graph, ContextContainer frameData)
            {
                ...
                var p = camera.isSceneViewCamera ? SceneSettings(owner.sceneViewProfile) :
                    (rain != null && rain.isActiveAndEnabled ? rain.ResolveSettings() : null);
                ...
                using (var builder = graph.AddRasterRenderPass<PassData>("Volumetric Fog and Rain", out var data))
                {
                    ...
                    data.premultiplied = owner.outputMode == FogOutputMode.Premultiplied;
                    bool independentDistance = data.fogOnly || p.fogDistanceMode == FogDistanceMode.Independent;
                    float fogEnd = data.fogOnly ? Mathf.Max(1, p.maxDistance) : Mathf.Max(1, p.fogMaxDistance);
                    data.fogDistance = new Vector4(Mathf.Max(0, p.fogStartDistance), Mathf.Max(0, p.fogDistanceFade), independentDistance ? 1 : 0, fogEnd);
                    data.fogHeight = new Vector4(p.fogBaseHeight, Mathf.Max(0, p.fogHeightFalloff), Mathf.Max(0, p.directionalScattering), Mathf.Clamp(p.scatteringAnisotropy, -.9f, .9f));
                    data.lightColor = Vector4.zero; data.lightDirection = new Vector4(0,1,0,0);
                    if (p.directionalScattering > 0)
                    {
                        ...
                        if (lights.mainLightIndex >= 0)
                        {
                            ...
                            {
                                data.lightColor = light.finalColor;
                                data.lightDirection = -light.localToWorldMatrix.GetColumn(2);
                            }
                        }
                    }
                    ...
                    data.volumes.feather = volumeScratch.feather;
                    System.Array.Copy(volumeScratch.rainFeathers, data.volumes.rainFeathers, volumeScratch.rainCount);
                    System.Array.Copy(volumeScratch.dryFeathers, data.volumes.dryFeathers, volumeScratch.dryCount);
                    ...
                    float noiseScale = Mathf.Max(.001f, p.noiseScale);
                    data.fogOffset = camera.isSceneViewCamera ? SceneFogOffset(p.fogNoiseVelocity, noiseScale, time) : rain.EvaluateFogOffset(p.fogNoiseVelocity, noiseScale, time);
                    data.advancedFog = data.premultiplied || p.fogHeightFalloff > 0 || p.directionalScattering > 0 ||
                        ((Vector3)data.fogOffset).sqrMagnitude > 0 || p.fogStartDistance > 0 || p.fogDistanceFade > 0 ||
                        (!data.fogOnly && independentDistance);
                    ...
                    float sceneRange = data.fogOnly ? Mathf.Max(1, p.maxDistance) : Mathf.Max(far, p.maxDistance);
                    if (independentDistance && p.HasFog) sceneRange = Mathf.Max(sceneRange, fogEnd);
                    data.distances = new Vector4(near, mid, far, sceneRange);
                    ...
                    builder.SetRenderFunc((PassData d, RasterGraphContext context) =>
                    {
                        ...
                        CoreUtils.SetKeyword(m, "_RAIN_FEATHER", d.useVolumes && d.volumes.feather);
                        CoreUtils.SetKeyword(m, "_FOG_ADVANCED", d.advancedFog);
                        m.SetInt("_FogPremultiplied", d.premultiplied ? 1 : 0);
                        m.SetVector("_FogDistance", d.fogDistance); m.SetVector("_FogHeight", d.fogHeight);
                        m.SetVector("_FogOffset", d.fogOffset); m.SetVector("_FogLightColor", d.lightColor);
                        m.SetVector("_FogLightDirection", d.lightDirection);
                        ...
                        if (d.useVolumes)
                        {
                            ...
                            if (d.volumes.feather)
                            {
                                m.SetVectorArray("_RainBoxFeathers", d.volumes.rainFeathers);
                                m.SetVectorArray("_RainDryFeathers", d.volumes.dryFeathers);
                            }
                        }
