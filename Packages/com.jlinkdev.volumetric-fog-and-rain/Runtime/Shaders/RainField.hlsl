...
float RainDensityNoise(float3 p)
{
...
float RainDensity(float3 world) { return RainDensityNoise(world * _RainNoise.x); }
float FogDensity(float3 world)
{
    #if defined(_FOG_ADVANCED)
        float density = RainDensityNoise(world * _RainNoise.x - _FogOffset.xyz);
        [branch] if (_FogHeight.y > 0)
            density *= exp(-min(80.0, max(0, world.y - _FogHeight.x) * _FogHeight.y));
    #else
        float density = RainDensity(world);
    #endif
    #if defined(_RAIN_VOLUMES) && defined(_RAIN_FEATHER)
        density *= RainVolumeMask(world);
    #endif
    return density;
}
    ...
    float mid, float far, float footprintOrigin, float footprintSlope, int layer, float begin, inout int remaining, inout float cost)
{
    ...
    [loop] for (int j = 0; j < _RainSteps && enter < end; j++)
    {
        ...
        if (presence > 0 && exit > enter)
        {
            ...
            {
                float3 world = worldOrigin + worldRay * t;
                float density = RainDensity(world);
                #if defined(_RAIN_VOLUMES) && defined(_RAIN_FEATHER)
                    density *= RainVolumeMask(world);
                #endif
                sum += coverage * presence * fade * density * lerp(0.6, 1.4, RainRandom(RainMix(h+6u)));
            }
        }
        if (exit >= end) break;
