# Volumetric Fog and Rain manual
...
2. Select the renderer data asset used by the camera. Run **Tools > jlinkdev >
   Volumetric Fog and Rain > Add Feature to Selected Renderer**, or add **Rain Renderer
...
3. Create a profile with **Assets > Create > jlinkdev > Volumetric Fog and Rain > Profile**.
...
run **Tools > jlinkdev > Volumetric Fog and Rain > Create Rain Laboratory**. It creates a unique
folder under `Assets/VolumetricFogAndRain` containing a scene, profile, material, renderer,
...
Import **Fog Laboratory**, or run **Tools > jlinkdev > Volumetric Fog and Rain > Create Fog
...
Import the **Volume Laboratory** sample, or run **Tools > jlinkdev > Volumetric Fog and Rain > Create Volume Laboratory**, then press Play for a dry-canopy demonstration.
...
See [fog distribution and light](fog.md), [soft boundaries](volumes.md),
[render timing and transparent output](rendering.md), and
[runtime settings and blending](runtime-settings.md).
