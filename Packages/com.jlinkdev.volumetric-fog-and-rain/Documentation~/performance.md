...
The Unity `Volumetric Fog and Rain` GPU profiling scope is measured separately from CPU
...
`Assets/PackageDevelopment/VolumetricFogAndRain/Benchmark/RainBenchmark.cs`. Its Reference
...
## 0.4.0 feature cost and comparison — 2026-09-25

Optional feathering and advanced fog have separate shader keywords. With both
absent, the shader omits the new box-mask loops, fog height/movement controls,
directional phase function, and premultiplied-output math. Fog Only still removes
all streak traversal. Enabling these controls has a cost; Fog Samples remains the
quality/performance control for density integration.

A same-session comparison used the previous 0.3.0 shader and final 0.4.0 shader
with the same current renderer/harness, Unity 6000.0.58f1, URP 17.0.3, D3D11,
RTX 2060 SUPER, 1920x1080, 120 warm-up frames, and 300 measured frames per case.
The project used Gamma color space to match the historical benchmark. These are
GPU-scope medians, not whole-frame timings:

| Case | Previous shader | Current shader |
| --- | ---: | ---: |
| Default combined rain/fog | 8.4948 ms | 9.0481 ms |
| Combined, density noise disabled | 8.4380 ms | 8.8965 ms |
| Global Fog Only | 0.7844 ms | 1.3363 ms |
| Bounded Fog Only | 0.6513 ms | 0.6574 ms |

This session had substantial timing variability (default p95 11.0126/11.4711 ms).
Even the previous shader was slower than its earlier recorded 5.7505 ms median.
These measurements do not establish an unchanged-cost claim or a precise isolated
regression. Reprofile on the consuming project's hardware/workload. The current
and previous shader images were byte-identical for all four compared cases; the
initial feature implementation also matched all eleven historical default cases.

Raw records: [previous](performance-data/Comparison040-Previous030.csv),
[current](performance-data/Comparison040-Current040.csv). Reproduce with the
host benchmark's `RAIN_BENCH_VARIANT=Compare040`; its Previous030 shader is a
preserved development-only baseline. The regular benchmark also includes
rain_feather, fog_feather, and fog_features for evaluating enabled features.
