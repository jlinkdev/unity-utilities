...
Volumetric Fog and Rain. Adjust Center and Size in local units, or resize with Scene view
...
- Feather Distance defaults to zero (hard cuts). Arbitrary mesh/SDF volumes are
...
In rain-linked Rain And Fog mode, haze integrates each wet interval after the near/mid transition. Independent fog ignores that transition.
...
jlinkdev > Volumetric Fog and Rain > Create Volume Laboratory**. Open the scene alone and
...
Set **Feather Distance** on each inclusion or exclusion box, in world metres.
Inclusions fade inward from their boundary. Exclusions remain fully clear within
the core and fade outward into the surrounding effect. Overlap uses the strongest
inclusion/exclusion mask rather than adding density. The 1 mm exclusion tolerance
still seals flush surfaces. Rotation and nonuniform scale preserve per-face world
units; the outer exclusion shell is included in CPU culling.

Zero on all visible boxes selects the original hard-boundary shader path. Feathering
adds per-sample/per-streak box-mask work only when enabled. Hard core intervals
still clip traversal; the feather shell attenuates contributions in the remaining
intervals. Increase Fog Samples for thin ramps or strongly varying height/noise.
The box dimensions remain the authored outer inclusion boundary / clear exclusion
core, so resizing does not silently move the physical boundary by half a feather.
