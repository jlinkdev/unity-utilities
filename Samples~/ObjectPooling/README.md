...
4. Create one or more `GameObjectPoolDefinitionSet` assets. Add one definition for each prefab, including its initial capacity and max capacity.
5. Add `GameObjectPoolRegistry` to `Pool Registry`. Assign the definition set assets and `Inactive Pooled Objects` as the default inactive parent. Configure shared activate/deactivate behavior and runtime fallback capacities on the registry.
6. Add `ObjectPoolingDemoController` to `Demo Controller`. Assign the pool handle, pool registry, active parent, spawn center, and the registry prefabs array. The registry prefab colors array controls which color each registry prefab uses while spawned, making a cube/sphere/capsule setup easy to distinguish.
7. Add `ObjectPoolingDemoOverlay` to `Demo Overlay`. Assign the demo controller, and optionally assign the handle and registry.

The included sample assets are organized into `Definitions`, `Scenes`, `Prefabs`, and `Scripts`. The prefabs use their default material and runtime color overrides, so no material assets are required.

If the registry is asked to spawn a prefab that is not present in any assigned definition set or scene-local definition, it logs a warning and creates a runtime pool using the registry fallback capacities.
